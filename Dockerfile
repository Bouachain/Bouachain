...
# then log output of file /code/bin/bouachain
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/bouachain \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/bouachain | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/bouachain /usr/bin/bouachain
