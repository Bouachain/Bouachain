...
HTTPS_GIT := github.com/bouachain/bouachain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=bouachain \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=bouachain \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/bouachain ./cmd/bouachain
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/bouachain.exe ./cmd/bouachain
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/bouachain
...
draw-deps:
	...
	@goviz -i ./cmd/bouachain -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c bouachain --local -f chains.yaml
endif
...
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | bouachain keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | bouachain keys add acc1 --recover`
